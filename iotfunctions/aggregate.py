...
class Sum(SimpleAggregator):
    ...
    def execute(self, group, scope=None):
        if group.isna().all():
            return None
        ...
        if aligned_cycle_start < aligned_cycle_end:
            ...
            if aligned_calc_start < aligned_cycle_end:
                ...
                if input_data_item.get('type') == 'METRIC':
                    ...
                    sql_quoted_timestamp_col_name = dbhelper.quotingColumnName(check_sql_injection(self.dms.eventTimestampColumn))
                    sql_quoted_device_id_col_name = dbhelper.quotingColumnName(check_sql_injection(self.dms.entityIdColumn))
                    sql_quoted_input_data_item_col_name = dbhelper.quotingColumnName(check_sql_injection(input_data_item['columnName']))
