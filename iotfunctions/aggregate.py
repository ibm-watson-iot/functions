...
class MsiHierarchyOccupancyCount(DirectAggregator):

    KPI_FUNCTION_NAME="MSI_HierarchyOccupancyCount"

    BACKTRACK_IMPACTING_PARAMETER = "start_of_calculation"

    def __init__(self, child_occupancy_count, start_of_calculation=None, name=None):
        super().__init__()
        self.logger = logging.getLogger('%s.%s' % (self.__module__, self.__class__.__name__))

        if child_occupancy_count is not None and len(child_occupancy_count) > 0:
            self.child_occupancy_count = child_occupancy_count
        else:
            raise RuntimeError(f"Function {self.KPI_FUNCTION_NAME} requires the parameter child_occupancy_count "
                               f"but it is empty: child_occupancy_count={child_occupancy_count}")
