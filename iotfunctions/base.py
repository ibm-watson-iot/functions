...
class DataExpanderTransformer(BaseTransformer):
    """
    Allows loading historic data for training and unsupervised anomaly detection.
    ...
    def expand_dataset(self, df_copy, limit=200): 
        ...
        start_ts = pd.Timestamp.now() - pd.Timedelta(days=7)   # not more than a week of data (no longer used, replaced by limit)
        ...
        try:
            ...
            if raw_input_items:   # list is not empty
                try:
                    ...
                    query_raw = query_raw.limit(limit)
            ...
            if derived_input_items:  # list is not empty
                ...
                try:
                    ...
                    query_dm = query_dm.filter(db.get_column_object(table_dm, 'KEY').in_(derived_input_items)).limit(limit)
                ...
                # rename 'value_n' to KEY row by row
