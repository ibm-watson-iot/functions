...
class Database(object):
    ...
    def __init__(self, credentials=None, start_session=False, echo=False, tenant_id=None, entity_metadata=None,
        ...
        if 'sqlite' in self.credentials:
            ...
            if db_type_from_env is not None and len(db_type_from_env) > 0:
                ...
                if db_type_from_env == 'DB2':
                    ...
                    raise ValueError(
                        'The database type \'%s\' is unknown. Supported type is DB2' % db_type_from_env)
            else:
                raise ValueError('The variable DB_CONNECTION_STRING was found in the OS environement but the variable '
                                 'DB_TYPE is missing. Possible value for DB_TYPE is DB2')
        else:
        ...
        if is_icp is not None and is_icp == 'true':
            # Check for server.crt in container path (mounted from host resources/)
            if os.path.exists('/var/www/as-pipeline/server.crt'):
                self.http = urllib3.PoolManager(timeout=urllib3.Timeout(connect=10, read=60), cert_reqs='CERT_REQUIRED',
                                                ca_certs='/var/www/as-pipeline/server.crt')
            elif os.path.exists('/secrets/iot-common/ca.crt'):
                self.http = urllib3.PoolManager(timeout=urllib3.Timeout(connect=10, read=60), cert_reqs='CERT_REQUIRED',
    ...
    def connection(self):
        """
        Property to provide backward compatibility for code that expects db.connection.
        Returns the SQLAlchemy engine which supports execute() operations.
        Note: For Table() constructor, use db.metadata directly instead.
        """
        return self.engine
    ...
    def close(self):
        ...
        This method provides cleanup for database resources.
        """
        try:
    ...
    def set_isolation_level(self, engine):
        if self.db_type == 'db2':
            with engine.connect() as con:
                con.execute(text('SET ISOLATION TO DIRTY READ;'))
