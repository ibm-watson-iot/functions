...
class LoadTableAndConcat(BaseLoader):
    ...
    def metadata(cls):
        return _generate_metadata(cls, {
            ...
            'input': [
                ...
                {
                    'name': 'columns',
                    'description': 'Names of table columns to load (comma separated).',
                    'type': 'CONSTANT',
                    'required': True,
                    'dataType': 'ARRAY',
                    'dataTypeForArray': ["LITERAL"],
                    'jsonSchema': {"$schema": "http://json-schema.org/draft-07/schema#","title": "columns","type": "array","minItems": 1,"items": {"type": "string"}}
                },
                {
                    'name': 'where_clause',
                    'description': 'The where clause, excluding the time range filtering.',
                    'type': 'CONSTANT',
                    'required': False,
                    'dataType': 'LITERAL'
                },
                {
                    'name': 'parse_dates',
                    'description': 'Names of table columns to parse as dates (comma separated).',
                    'type': 'CONSTANT',
                    'required': False,
                    'dataType': 'ARRAY',
                    'dataTypeForArray': ["LITERAL"],
                ...
                {
                    'name': 'id_col',
                    'description': 'Name of the column containing the entity ID.',
                    'type': 'CONSTANT',
                    'required': True,
                    'dataType': 'LITERAL'
                },
                {
                    'name': 'timestamp_col',
                    'description': 'Name of the column containing the time series base timestamp.',
                    'type': 'CONSTANT',
                    'required': True,
                    'dataType': 'LITERAL'
                },
            ...
            'output': [
                {
                    'name': 'names',
                    'dataTypeFrom': 'columns',
                    'cardinalityFrom': 'columns',
                    'description': 'New data item names to create (data items are comma separated).'
                }
            ],
            'tags': [
                'JUPYTER'
    ...
    def __init__(self, table=None, columns=None, names=None, where_clause=None, parse_dates=None, id_col=None, timestamp_col=None):
        self.logger = logging.getLogger('%s.%s' % (self.__module__, self.__class__.__name__))
        ...
        self.parse_dates.add(self.timestamp_col)
    ...
    def execute(self, df, start_ts, end_ts, entities=None):
        ...
        sql = 'SELECT %s, %s AS %s, %s AS %s FROM %s' % (
            ', '.join([dbhelper.quotingColumnName(col) for col in self.columns]), 
            dbhelper.quotingColumnName(self.id_col), 
            dbhelper.quotingColumnName(key_id), 
            dbhelper.quotingColumnName(self.timestamp_col), 
            dbhelper.quotingColumnName(key_timestamp), 
            dbhelper.quotingTableName(self.table))
        ...
        if start_ts is not None and end_ts is not None: # TODO start_ts and end_ts are expected to be not None
            if not condition_applied:
                ...
                sql += ' AND '
            sql += "%s < %s AND %s <= %s" % (dbhelper.quotingSqlString(start_ts), dbhelper.quotingColumnName(self.timestamp_col), dbhelper.quotingColumnName(self.timestamp_col), dbhelper.quotingSqlString(end_ts))
            condition_applied = True
        if entities is not None:
            if not condition_applied:
                ...
                sql += ' AND '
            sql += "%s IN (%s)" % (dbhelper.quotingColumnName(self.id_col), ', '.join([dbhelper.quotingSqlString(ent) for ent in entities]))

        self.logger.debug('sql=%s' % sql)
        
        df_sql = pd.read_sql(sql, self._get_dms().db_connection_dbi)
        df_sql = df_sql.astype(dtype={self.id_col: str, key_id: str}, errors='ignore')
        ...
        # rename loaded column to the specified data item names
        column_renaming = {}
        for idx, name in enumerate(self.names):
            column = df_sql.columns[idx]
            column_renaming[column] = name
        df_sql = df_sql.rename(columns=column_renaming)
