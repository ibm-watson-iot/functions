...
1.  In [Github](https://github.com/), create a private empty repository <br>
![create-empty-repo](./readme-images/create_empty_repo.gif)
2. Create and save [personal access token](https://docs.github.com/en/github/authenticating-to-github/keeping-your-account-and-data-secure/creating-a-personal-access-token)
...
1. Right click on your project `my-custom-functions` in the project window
2. Select Open In >> Terminal
3. In the terminal, follow this sequence of commands to get the starter code into your empty private repository's 
main 
  branch
```bash
git remote -v            
...
1. Add a `dev_resources` folder in the working directory. You can use this to store credentials required later. _Make 
sure this folder is in `.gitignore` file_
...
**OPTION 1** Use random data for testing locally, as shown in 
[local_test_using_random_data.py](./scripts/local_test_using_random_data.py) <br>
**OPTION 2** Use .csv file data for testing locally, as shown in 
[local_test_using_csv_data.py](./scripts/local_test_using_csv_data.py)
...
1. On the `Data` tab select the `create new` button
2. Search for your registered custom-function
3. Provide the required input and output
4. Create the data item/s
5. The pipeline will run within next 5 min with the default schedule


-----------
## Debugging In Pipeline

This step is necessary if the UI displays `Analysis Error` after adding a metric that uses the new 
custom-function. The error message is a result of a runtime error encountered by the pipeline.
With the steps show below you can download the pipeline's runtime log file and perform debugging analysis locally to 
...
3. If the debugging suggests an error in the custom-function, and you want to fix the function follow these steps 
    ...
    3. Fix and push the new and improved custom-function to github
    4. Register the new custom-function
    5. Verify in the UI again
